...
def test_for_lection_6_task_2(driver):
    ...
    action.send_keys_to_element(quantity, Keys.CONTROL+"a")
    action.send_keys_to_element(quantity, Keys.DELETE)
    action.send_keys_to_element(quantity, "123456")
    action.perform()
    ...
    action1.click(price)
    action1.send_keys_to_element(price, Keys.CONTROL+"a")
    action1.send_keys_to_element(price, Keys.DELETE)
    action1.send_keys_to_element(price, "5")
    action1.perform()

    price_u = driver.find_element_by_name("gross_prices[USD]")
    action2 = ActionChains(driver)
    action2.click(price_u)
    action2.send_keys_to_element(price_u, Keys.CONTROL+"a")
    action2.send_keys_to_element(price_u, Keys.DELETE)
    action2.send_keys_to_element(price_u, "6")
    action2.perform()

    price_e = driver.find_element_by_name("gross_prices[EUR]")
    action3 = ActionChains(driver)
    action3.click(price_e)
    action3.send_keys_to_element(price_e, Keys.CONTROL+"a")
    action3.send_keys_to_element(price_e, Keys.DELETE)
    action3.send_keys_to_element(price_e, "7")
    action3.perform()
